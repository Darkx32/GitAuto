...
use crate::core::git::git_helper::{self, get_git_data};
...
pub fn create_checkout(name: String) -> color_eyre::Result<()> {
    let git_data = get_git_data()?;
    let head = git_data.repo.head()?;
    let target_commit = head.peel_to_commit()?;

    git_data.repo.branch(&name, &target_commit, false)?;

    let ref_name = format!("refs/heads/{}", name);
    git_data.repo.set_head(&ref_name)?;

    let mut checkout_builder = CheckoutBuilder::new();
    checkout_builder.safe();
    git_data.repo.checkout_head(Some(&mut checkout_builder))?;
...
pub fn amend_last_commit(new_msg: String) -> color_eyre::Result<()> {
    ...
    let tree = git_data.repo.find_tree(tree_oid)?;

    let sig = git_data.repo.signature()?;
...
pub fn get_all_lines_changed(filter: Option<Vec<String>>) -> color_eyre::Result<Vec<String>> {
    ...
    let diff = git_data.repo.diff_index_to_workdir(None, Some(&mut opts))?;
